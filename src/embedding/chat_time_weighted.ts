 ...
 * ```
 */
export class ChatTimeWeightedVectorStoreRetriever extends BaseRetriever {
  static lc_name() {
    return "ChatTimeWeightedVectorStoreRetriever";
  }
  ...
  async addDataSet(url: string): Promise<void> {
    console.log("addDataSet: " + url)

    const embeddingKey = `embedding:${toBase64(url)}}`
    const docKey = `embedding_doc:${toBase64(url)}}`

    const {[embeddingKey]: cacheEmbeddings} = await Browser.storage.local.get(embeddingKey)

    const {[docKey]: cacheDocs} = await Browser.storage.local.get(docKey)

    const now = Math.floor(Date.now() / 1000);
    const savedDocs = this.prepareDocuments(cacheDocs, now);

    this.memoryStream.push(...savedDocs);
    await this.vectorStore.addVectors(cacheEmbeddings, savedDocs);
  }

  async addChatHistoryDataSet(texts: string[]): Promise<void> {
    const cacheDocs = [] as Document[]
    const cacheEmbeddings = [] as number[][]
    console.log("chat add documents: " + texts)
    ...
    for (let i = 0; i < texts.length; i++) {
      ...
      for (const c of Array.from(historyQuery).reverse()) {
        ...
        for (const m of messages) {
          const msgKey = `embedding:${m.mark}:${m.id}`
          console.log("msgKey: " + msgKey)
          const {[msgKey]: msgEmbedding} = await Browser.storage.local.get(msgKey)
          if (msgEmbedding) {
            cacheDocs.push(new Document({pageContent: m.text}))
            cacheEmbeddings.push(msgEmbedding)
            console.log("add text: " + m.text)
          }
        }
      }
    }
